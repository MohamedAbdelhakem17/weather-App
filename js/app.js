...
async function weather(place) {
  let weatherApi = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=46660025d01442d581f51515221310&q=${place}&days=3`,
    { method: "GET" }
  );
...
function weatherToDay(location, current) {
  ...
  <div class="box">
    ...
    <div class="body py-5 px-2">
      ...
      >
        <div><img src="img/icon-compass.png" class="px-2" />${
          current.wind_dir
        }</div>
        <div><img src="img/icon-wind.png" class="px-2" />${
          current.wind_mph
        }km/h</div>
        <div>
